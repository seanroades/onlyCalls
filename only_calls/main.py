...
if not OPENAI_API_KEY or not ELEVEN_LABS_API_KEY or not SUPABASE_ADMIN or not SUPABASE_URL:
    raise ValueError("Both OPENAI_API_KEY and ELEVEN_LABS_API_KEY and SUPABASE_ADMIN and SUPABASE_URL must be set in the environment.")
...
def upload_reminder(audio: Union[bytes, Iterator[bytes]], number: str, time: str, bot_name: str) -> bool:
    ...
    supabase.storage.from_("reminders").upload(file=audio, path=path_in_bucket, file_options={"content-type": "audio/mpeg"})

    data, count = supabase.table('reminders').insert([
    {
        'number': number, 
        'time': time, 
        'bot_name': bot_name, 
...
def time_safety_checks(time_json):
    ...
    try:
        ...
        human_readable_time = datetime_object.strftime('%H:%M on %B %d, %Y') # B for full month name
        ...
        new_date_time_object = new_date_time_object.astimezone(pytz.timezone('America/Los_Angeles')) # TODO: generalize timezon

        timestamp = datetime_object.timestamp()
        la_timestamp = new_date_time_object.timestamp() # LA time
        datetime_object = datetime.fromtimestamp(timestamp, pytz.UTC)
        new_date_time_object = datetime.fromtimestamp(la_timestamp, pytz.timezone('America/Los_Angeles')) 
        # Cast as str b/c Object of type datetime is not JSON serializable
        return {"human_readable": human_readable_time, "timestampz": str(new_date_time_object), "utc_timestamp": str(datetime_object)} 
...
def create_reminder(goal: str, time: str, phone_number: str, exact_time: any, bot_name: str) -> bool:
    ...
    upload_reminder(audio, phone_number, exact_time, bot_name)
